...
function search(page) {
	...
	$.ajax({
		...
		async : false,
		url : '<%=basePath%>/stockOrdering/getStockOrderingCount.action', // 目标地址
		data : pro,
...
function InitData(pageindx, pagenum, pc) {
	...
	$.ajax({
		type : "POST", // 用POST方式传输
		dataType : "json", // 数据格式:JSON
		url : '<%=basePath%>/stockOrdering/getStockOrderingPage.action', // 目标地址
		...
		success : function(json) {
			...
			$.each(json, function(i, n) {
				...
				html += "<td><input type='checkbox' class='cb'   value='"+n.stockOrderingID+"' /></td>";
				html += "<td>"+(i+1)+"</td>";
				html += "<td>"+ n.stockOrderingID +"</td>";
				...
				html += "<td><a class='btn_set btn016' style='color:green' onclick='oper(" + n.systemID + ","+n.stockOrderingID+")'>操作</a></td>";
				html += "</tr>";
...
function searchOrder(){
	...
	<input type="hidden" name="userName" id="userName" value="${userName}" />
		...
		<nav class="breadcrumb">
			...
			<span class="c-gray en">&gt;</span> 配置采购定单列表 
		...
		<div class="pd-20">
			...
			<label class="dialog_label">客&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;户：</label>
				...
				<input type="hidden" id="stockOrderingID" name="stockOrderingID"/>
		...
		function oper(supplierID,stockOrderingID){
			$("#box1").hide();
			$("#table1").hide();
			$("#dialogdiv").hide();
			$("#box2").show();
			$("#table2").show();
			getOrderingItemList(supplierID,stockOrderingID);
		}
		...
		function getOrderingItemList(systemID,stockOrderingID) {
			
			var pro = "&systemID="+systemID+ "&stockOrderingID="+stockOrderingID;
			$.ajax({
				type : "POST", // 用POST方式传输
				dataType : "json", // 数据格式:JSON
				url : '<%=basePath%>/stockOrdering/getOrderingItemList.action', // 目标地址
				data : pro ,
				success : function(json) {
					...
					// 遍历查询到的数据,输入前台
					var stockOrderingID=0;
					$.each(viewOrderingItemList, function(i, n) {
						var html = "";
						stockOrderingID=n.stockOrderingID;
						//明细
						html += "<tr class='text-c'>";
...
// 							html += "<td>" +(new Date(n.shelfLife)).Format('yyyy-MM-dd') + "</td>";	
						...
						html += "<td><a class='btn_set btn023' href='javascript:;'  onclick='fileUpload("+n.stockOrderingID+","+n.orderingItemID+")'>上传</a><a class='btn_set btn034' href='javascript:;'  onclick='filedetail("+n.orderingItemID+")'>索证 详情</a></td>";
						html += "</tr>";
						
						$(".orderingItemTable").append(html);
					});
					$("#stockOrderingID").val(stockOrderingID);
		...
		//打印
		function print(){
			var strReportFile = "OrderingNoSumBySupplier.raq";
			var supplierID=$("#supplierID").val();
			var systemID=$("#systemID").val();
			var begintime=$("#begintime").val();
			var endtime=$("#endtime").val();
		...
		function configure(){
			...
			var systemID=$("#systemID").val();
			var stockOrderingID=$("#stockOrderingID").val();
			var supplierID=$("#supplierID").val();
			$.ajax({
				type : "POST", // 用POST方式传输
				dataType : "json", // 返回数据格式:JSON
				async : false,
				url : "<%=basePath%>/stockOrdering/configureDate.action", // 目标地址
				data : {array:array2,stockOrderingID:stockOrderingID,systemID:systemID,supplierID:supplierID},					
				success : function(data) {
		...
		function fileUpload(stockOrderingID,orderingItemID){
				...
				if(file.length>0){
                		layer.confirm('确认要提交吗？',function(index){
                			...
                			var options={
                					type:"POST",
                					dateType:"json",
                					url:"<%=basePath%>/stockOrdering/saveCertificatesOrdering.action",
                					data:{systemID:systemID,stockOrderingID:stockOrderingID,supplierID:supplierID},
                					success:function(data){
		...
		function filedetail(orderingItemID){
			...
			$.ajax({
				...
				dataType : "json", // 返回数据格式:JSON
				async : false,
				url : "<%=basePath%>/stockOrdering/checkCertificates.action", // 目标地址
				data : {orderingItemID:orderingItemID,stockOrderingID:stockOrderingID,systemID:systemID,supplierID:supplierID},					
				success : function(json) {
					if(json=="0"){
						...
						var url1="<%=basePath%>/stockOrdering/showCertificates.action?supplierID="+supplierID+"&systemID="+systemID+"&stockOrderingID="+stockOrderingID+"&orderingItemID="+orderingItemID;
