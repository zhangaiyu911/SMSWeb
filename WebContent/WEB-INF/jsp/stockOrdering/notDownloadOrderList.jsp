...
function search(page) {
	...
	$.ajax({
		...
		async : false,
		url : '<%=basePath%>/stockOrdering/getNotDownloadOrderingCount.action', // 目标地址
		data : pro,
...
function InitData(pageindx, pagenum, pc) {
	...
	$.ajax({
		type : "POST", // 用POST方式传输
		dataType : "json", // 数据格式:JSON
		url : '<%=basePath%>/stockOrdering/getNotDownloadOrderingPage.action', // 目标地址
		...
		success : function(json) {
			...
			$.each(json, function(i, n) {
				...
				else if (4==n.orderingFlag) {
					html += "<td><span class='c-red'>作废</span></td>";
				}
				else{
					html += "<td>错误的状态，请修改</td>";
				}
				html += "<td><a class='btn_set btn016' style='color:green' onclick='oper(" + n.systemID + ","+n.stockOrderingID+","+n.itemID+")'>详情</a></td>";
				html += "</tr>";
...
function searchOrder(){
	...
	<input type="hidden" name="supplierID" id="supplierID" value="${supplierID}" />
		...
		<nav class="breadcrumb">
			...
			<span class="c-gray en">&gt;</span> 下载采购定单列表 
		...
		<div class="pd-20">
			...
			<div class="dialog_div" id="dialogdiv"><label class="dialog_label">客&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;户：</label>
		...
		function oper(supplierID,stockOrderingID,itemID){
			$("#box1").hide();
			$("#table1").hide();
			$("#dialogdiv").hide();
			$("#box2").show();
		...
		function downloadOrder(){
			if($(".cbc:checked").length==0){
				...
				layer.confirm('确认要下载吗？',function(index){
					url="<%=basePath%>/stockOrdering/downloadOrdering.action?itemID=";
		...
		function downloadOrder2(){
				layer.confirm('确认要下载吗？',function(index){
					...
					url="<%=basePath%>/stockOrdering/downloadOrdering.action";
					$.ajax({
						type :"POST",
						dataType:"json",
						async:false,
