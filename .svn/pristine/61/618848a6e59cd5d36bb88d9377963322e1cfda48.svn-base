...
function getUserRoleDepartmentList(){
	...
	$.ajax({
		...
		async : false,
		url : "<%=basePath%>/user/userRoleAuthConfigInit.action", // 目标地址
				...
				for(var i=0;i<data.deptList.length;i++){
					...
					for(var j=0;j<data.userRoleDeptList.length;j++){
						if(data.deptList[i].departmentID==data.userRoleDeptList[j].departmentID){
							html += "<td><input type='checkbox' class='cb' value='"+data.deptList[i].departmentID+"' checked='checked'/></td>";
							html += "<td><span class='c-green'>"+ data.deptList[i].departmentName +"</span></td>";
							flag=1;
						}
					}
					if(flag==0){
						html += "<td><input type='checkbox' class='cb' id ='"+data.deptList[i].departmentID+"' value='"+data.deptList[i].departmentID+"' /></td>";
					...
					if(i==data.deptList.length-1&&i%4!=3){
