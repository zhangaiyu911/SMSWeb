...
function search(page) {
	...
	$.ajax({
		...
		async : false,
		url : '<%=basePath%>/supplier/getSupplierCount.action', // 目标地址
		data : pro,
...
function InitData(pageindx, pagenum, pc) {
	...
	$.ajax({
		type : "POST", // 用POST方式传输
		dataType : "json", // 数据格式:JSON
		url : '<%=basePath%>/supplier/getSupplierPage.action', // 目标地址
		data : pro + "&page=" + page + "&limit=" + pagenum,
		success : function(json) {
			$(".supplierTable tr:gt(0)").remove();
			// 遍历查询到的数据,输入前台
			$.each(json, function(i, n) {
				...
				if(n.systemName==null){
					n.systemName="";
				}
				html += "<td>"+ n.systemName +"</td>";
				html += "<td>"+ n.wuliuSupplierID +"</td>";
				if(n.consignorName==null){
					n.consignorName="";
				}
				html += "<td>"+ n.consignorName +"</td>";
				if(n.consignorPhone==null){
					n.consignorPhone="";
				}
				html += "<td>"+ n.consignorPhone +"</td>";
				
				if (0==n.usingFlag) {
					html += "<td>待审核</td>";
				}else if (1==n.usingFlag) {
					html += "<td>通过</span></td>";
				}else if (2==n.usingFlag) {
					html += "<td><span class='c-red'>不通过</span></td>";
				}
				else{
					html += "<td>错误的状态，请修改</td>";
				}
				html += "<td class='text-l'> " + n.memo + "</td>";
				html += "<td><a class='btn_set btn016' style='color:green' onclick='oper(" + n.supplierID + ","+n.itemID+")'>操作</a></td>";
				html += "</tr>";
				
				$(".supplierTable").append(html);
			});
			//$('.supplierTable').alterBgColor();   //表格选中效果
			$('table').tableCheckbox();
...
function searchSupplier(){
	...
	<input type="hidden" name="errorcode" id="errorcode" value="${errorcode}" />
		...
		<nav class="breadcrumb">
			...
			<span class="c-gray en">&gt;</span> 供货商管理
			<span class="c-gray en">&gt;</span> 供货商列表 
...
<%-- 							<a class="btn_set btn015" href="javascript:;" onclick="post_add('添加','<%=basePath%>/position/editPositionInit.action?positionID=0','','600','450')">添加</a> --%>
<%-- 							<a class="btn_set btn016"  href="javascript:;" onclick="post_edit('编辑','<%=basePath%>/position/editPositionInit.action?positionID=','','600','450')">编辑</a> --%>
							
<!-- 							<a class="btn_set btn001"  href="javascript:;" onclick="supplier_del('不通过','','')" >不通过</a> -->
<!-- 							<a class="btn_set btn018"  href="javascript:;" onclick="supplier_open('通过','','')" >通过</a> -->
							
<!-- 	               		 	<a class="btn_set btn004" href="javascript:;"  id="dialog-link">查询</a> -->
<!-- 	               		 	<select name = "deptid" id="deptid"> -->
<%-- 							<c:forEach items="${deptList}" var="dept"> --%>
<%-- 								<option value = "${dept.departmentID}"  --%>
<%-- 									<c:if test="${dept.departmentID==departmentID}">selected="selected"</c:if>>${dept.departmentName}</option> --%>
<%-- 								</c:forEach> --%>
<!-- 							</select> -->
						</span> 
					...
					<div class="dialog_div">
						...
						<input type="text" class="select dialog_select" id="supplierName" name="supplierName" placeholder="请输入供货商名称"/>
					</div> 	
							...
							<table width="100%" class="supplierTable">
								<thead>
									<tr class="text-c">
										...
										<th width="12%">物流系统名称</th>
										<th width="12%">物流系统编号</th>
