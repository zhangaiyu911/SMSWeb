	...
	$.ajax({
		...
		success : function(json) {
			...
			$.each(json, function(i, n) {
				...
				html += "<td>"+ n.unit +"</td>";
				html += "<td>"+ n.instockprice.toFixed(2) +"</td>";
				html += "<td>"+ n.instockquantity.toFixed(2) +"</td>";
				html += "<td>"+ n.instockmoney.toFixed(2) +"</td>";
...
function searchInStockItem(){
	...
	<input type="hidden" name="inStockType" id="inStockType" value="${inStockType}" />
		...
		<nav class="breadcrumb">
			...
			<span class="c-gray en">&gt;</span> <c:if test="${inStockType==1}">食堂</c:if><c:if test="${inStockType==2}">采购</c:if>送货明细列表 
...
<!--                		 	<a class="btn_set btn004" href="javascript:;"  id="dialog-link">查询</a> -->
			...
			<div class="dialog_div">
				...
				<select class="select dialog_select" id="supplierID" name="supplierID" onchange="getSystem(this)">
			...
			var supplierID =obj.value;
			 if (supplierID!="0") {
					...
					$.ajax({
						dataType : "json", 
						async : false,
						url : "<%=basePath%>/inStock/getSystemList.action?supplierID="+supplierID, 
						success : function(data) {
							...
							for ( var i in data.systemSupplierList) {
								...
								options+='<option value="'+systemSupplier.systemID+'">'+systemSupplier.systemName+'</option>';
							}
							$('#systemID').html(options);
						},
						error : function(XMLHttpRequest, textStatus, errorThrown) {
