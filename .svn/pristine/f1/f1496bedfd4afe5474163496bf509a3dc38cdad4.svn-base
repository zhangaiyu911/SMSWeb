		...
		<form action="" method="post" class="form form-horizontal" id="formUser">
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-size:14px">
				  <tr>
					...
					&nbsp;
						 ...
						 <c:if test="${certificatesItemList.savePath!=''}">								 					  	         
			  	         <a href="<%=basePath%>${certificatesItemList.savePath}" target="_blank">
			  	         <img src="<%=basePath%>${certificatesItemList.savePath}" width="200" height="110" id="img${certificatesItemList.itemID}" />
			  	         </a>
						 </c:if>
				  ...
				  <tr>
				    ...
				    &nbsp;<input class="Wdate inputreadonly" maxlength="20" readonly="readonly" type="text" name="stopdate${certificatesItemList.itemID}" id="stopdate${certificatesItemList.itemID}" value="${qualificationsList.endDate}" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" datatype="*" nullmsg="到期日期不能为空" onchange="ischeckstopdate(${certificatesItemList.itemID},this.value);"/>
...
//本地文件上传
function upf(sqid){
	var url="<%=basePath%>/user/openImagesUploadInit.action?sqid="+sqid;
	layer_show('选择文件',url,'900','600');
	//window.open("../../modelPage/systemInit/supplier.do?method=openImagesUploadInit&sqid="+sqid,"_blank", "height=550,width=700,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=no");			
